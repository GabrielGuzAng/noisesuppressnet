...
def compute_metrics(reference_audio, processed_audio, sr=SAMPLE_RATE):
    """Devuelve dict con PESQ-NB, STOI, SI-SDR."""
    ...
    try:
        ...
        pesq_nb = np.nan  # PESQ puede fallar en clips muy cortos
    ...
    return {"PESQ-NB": pesq_nb, "STOI": stoi_val, "SI-SDR": sisdr_val}
...
if __name__ == "__main__":
    ...
    # Evaluar 4 condiciones
    print("Computing metrics for all conditions...")
    
    # 1. Noisy (baseline absoluto)
    df_noisy = evaluate_directory(
        PROJECT_ROOT / "data" / "estimates" / "v0",
        ref_filename="clean.wav", proc_filename="noisy.wav"
    )
    
    # 2. Butterworth
    df_butter = evaluate_directory(
        PROJECT_ROOT / "data" / "estimates" / "butterworth",
        ref_filename="clean.wav", proc_filename="filtered.wav"
    )
    
    # 3. CRN v0
    df_crn = evaluate_directory(
        PROJECT_ROOT / "data" / "estimates" / "v0",
        ref_filename="clean.wav", proc_filename="enhanced.wav"
    )
    
    # Tabla resumen
    summary = pd.DataFrame({
        "Condition": ["Noisy", "Butter 4kHz", "CRN v0"],
        "PESQ-NB": [df_noisy["PESQ-NB"].mean(), df_butter["PESQ-NB"].mean(), df_crn["PESQ-NB"].mean()],
        "STOI": [df_noisy["STOI"].mean(), df_butter["STOI"].mean(), df_crn["STOI"].mean()],
        "SI-SDR": [df_noisy["SI-SDR"].mean(), df_butter["SI-SDR"].mean(), df_crn["SI-SDR"].mean()],
    })
    summary.to_csv(PROJECT_ROOT / "results" / "summary_metrics.csv", index=False)
    print("\n" + str(summary))
